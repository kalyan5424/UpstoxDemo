...
const BottomView = () => {
  ...
  return (
    <TouchableWithoutFeedback onPress={onArrowPress}>
      <View style={styles.bottomViewContainer}>
        <View style={styles.arrowContainer}>
          {!expanded ? (
            <UpArrow testID='UpArrowIcon' style={styles.arrowStyle} />
          ) : (
            <UpArrow testID='DownArrowIcon' style={styles.downArrowStyle} />
          )}
        </View>
        ...
        {expanded && (
          <View>
            ...
            <BottomViewItem
              title={Labels.TOTAL_INVEST}
              value={`₹ ${calTotalInvestmentValue(data?.userHolding ?? [])}`}
            />
